    ...
    <div class="row">
        ...
        <div class="col-9">
            ...
            <div class="text-center">
                ...
                @endif
                @if(session('admin-info-update'))
                <div class="mt-3 fw-bold d-flex justify-content-center align-items-center"
                    style="height: 30px; font-size:13px; color:red; padding:10px; width:100%; background: black; border-radius: 20px">
                    {{ session('admin-info-update') }}
                </div>
                @endif
            </div>
            <div class="row" style="height: 90vh;">
                <div class="col-md-6">
                    <div class="d-flex align-items-center justify-content-center" style="height: 90vh;">
                        <div class="p-5"
                            ...
                            <img src="{{ asset('uploads/adminProfile/'.$admin->image) }}" alt="{{ $admin->name }}"
                            ...
                            <form action="{{route('adminDashboard')}}" method="post" enctype="multipart/form-data">
                                ...
                                <input type="text" hidden name="name" value="{{ $admin->name }}">
                                <input type="text" hidden name="role" value="{{ $admin->role }}">
                                <input type="text" hidden name="email" value="{{ $admin->email }}">
                                <input type="text" hidden name="password" value="{{ $admin->password }}">
                                <input type="text" hidden name="address" value="{{ $admin->address }}">
                                <input type="text" hidden name="phone" value="{{ $admin->phone }}">
                                <input type="text" hidden name="status" value="{{ $admin->status }}">
                                <button type="submit" class="btn btn-primary btn-sm mt-3">Set Photo</button>
                            </form>
                            @else
                            <br>
                            <h5 class="fw-bold mt-2">{{ $admin->name }}</h5>
                            <a href="{{ 'changeAdminImage/'.$admin->id }}" class="btn btn-danger btn-sm mt-3">Change
                                Photo</a>
                            @endif
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="d-flex align-items-center justify-content-center" style="height: 90vh;">
                        <div class="p-5"
                            style="border-radius: 10px; background: #EFE3D0; height: 400px; width:430px; box-shadow: 3px 3px 10px gray;">
                            <h4 class="mb-5 fw-bold text-danger text-uppercase">Admin Information</h4>
                            ...
                            <div>
                                <a href={{ "editAdminProfile/" .$admin->id}} class="btn btn-warning btn-sm">Edit
                                    Profile</a>
                                {{-- Delete account --}}
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <a class="btn btn-danger btn-sm" href={{ "deleteAdminAccount/"
                                                    .$admin->id }}>Delete Account</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
